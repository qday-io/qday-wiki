...
Cross-chain asset transfers between QDay Aevum and external chains use the **PolygonZkEVMBridge** architecture adapted for QDAY's UTXO-based L1.
...
**QDAY ↔ Abelian Bridge (PQZK Bridge)** — Assets moving between QDay Aevum (EVM) and the Abelian Layer 1 (UTXO) go through the PQZK Bridge, which is anchored by the Committer's status root submissions to Abelian Chain.
