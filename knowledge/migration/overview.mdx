...
Two different moves are happening at once. **Chain upgrade** is the network itself: QDay Origin → QDay Aevum. **Asset migration** is your tokens: lock on QDay Origin, receive 1:1 on QDay Aevum.

---

## 1. Chain upgrade

**QDay Origin** is today’s production chain. **QDay Aevum** is the upgraded network (new chain ID, new RPC, new explorer). Wallets and dApps must add QDay Aevum; they do not automatically follow the upgrade.

| | QDay Origin | QDay Aevum |
|---|---|---|
...
2. Point apps and RPC configs at the Aevum endpoint when you start building or testing on QDay Aevum — [Network Configuration](/Knowledge/technical/developer-guide/network-configuration).
...
After the new chain exists, balances still have to be moved. You lock an asset **once** on QDay Origin; the same amount is released to you **1:1** on QDay Aevum.

:::danger[One-way and permanent]
Locking on QDay Origin cannot be undone. There is no reverse bridge. Check the amount and the QDay Aevum recipient before you sign.
:::
...
```mermaid
flowchart LR
    A["Lock on QDay Origin"] --> B[Wait for confirmations]
    B --> C["Relayer allocates 1:1<br/>on QDay Aevum"]
    C --> D[Tokens received on QDay Aevum]
```

You sign **one** transaction: the lock on QDay Origin. The relayer pays gas on QDay Aevum.

| Contract | Chain | Job |
|---|---|---|
| MigrationLockbox | QDay Origin | Holds locked assets (one-way) |
| MigrationDistributor | QDay Aevum | Credits you 1:1 from the pre-funded pool |
| MigrationClaim | QDay Aevum | Backstop claim if the window ends without a relay |
...
| Asset | How |
|---|---|
| USD8 / WABEL / WQDAY | Lock the ERC-20 on QDay Origin |
