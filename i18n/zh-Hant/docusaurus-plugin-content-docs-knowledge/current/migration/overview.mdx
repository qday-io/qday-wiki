...
目前有兩件不同的事情同時進行。**鏈升級**指的是網路本身：QDay Origin → QDay Aevum。**資產遷移**指的是你的代幣：在 QDay Origin 上鎖定，並在 QDay Aevum 上以 1:1 收到。

---

## 1. 鏈升級

**QDay Origin** 是目前的正式生產鏈。**QDay Aevum** 是升級後的網路（新的鏈 ID、新的 RPC、新的區塊瀏覽器）。錢包與 dApp 必須手動新增 QDay Aevum；它們不會自動跟隨升級。
...
2. 當你開始在 QDay Aevum 上建置或測試時，將應用程式與 RPC 設定指向 Aevum 端點——[網路設定](/Knowledge/technical/developer-guide/network-configuration)。
