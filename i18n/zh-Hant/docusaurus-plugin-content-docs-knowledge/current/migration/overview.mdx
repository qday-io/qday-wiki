...
目前有兩件不同的事情同時進行。**鏈升級**指的是網絡本身：QDay Origin → QDay Aevum。**資產遷移**指的是你的代幣：在 QDay Origin 上鎖定，並在 QDay Aevum 上以 1:1 收到。

---

## 1. 鏈升級

**QDay Origin** 是目前的正式生產鏈。**QDay Aevum** 是升級後的網絡（新的鏈 ID、新的 RPC、新的區塊瀏覽器）。錢包與 dApp 必須手動新增 QDay Aevum；它們不會自動跟隨升級。

| | QDay Origin | QDay Aevum |
|---|---|---|
| 是什麼 | 目前的正式生產網絡 | 升級後的網絡 |
...
2. 當你開始在 QDay Aevum 上建置或測試時，將應用程式與 RPC 設定指向 Aevum 端點——[網絡設定](/Knowledge/technical/developer-guide/network-configuration)。
...
新的鏈存在之後，餘額仍需要被移動。你在 QDay Origin 上**一次性**鎖定資產；相同數量會在 QDay Aevum 上以 **1:1** 釋放給你。

:::danger[單向且永久]
在 QDay Origin 上鎖定後無法復原。沒有反向跨鏈橋。簽署前請確認金額與 QDay Aevum 的接收地址。
:::
...
你只需簽署**一筆**交易：在 QDay Origin 上的鎖定。中繼者（relayer）會支付 QDay Aevum 上的 Gas。

| 合約 | 鏈 | 職責 |
|---|---|---|
| MigrationLockbox | QDay Origin | 保管已鎖定的資產（單向） |
| MigrationDistributor | QDay Aevum | 從預先注資的資金池以 1:1 撥付給你 |
| MigrationClaim | QDay Aevum | 若遷移窗口結束仍未完成中繼時的備援領取機制 |
...
| 資產 | 方式 |
|---|---|
| USD8 / WABEL / WQDAY | 在 QDay Origin 上鎖定 ERC-20 |
